...
docker exec -it kyrix_db_1 psql postgresql://postgres:kyrixftw@localhost/graphvis -c "CREATE TABLE fbfriendsNodes(nodeId SERIAL, edgeCount INT, posX FLOAT8, posY FLOAT8, PRIMARY KEY(nodeId));"      # create Facilities table in recFut database
docker exec -it kyrix_db_1 psql postgresql://postgres:kyrixftw@localhost/graphvis -c "COPY fbfriendsNodes(nodeId, edgeCount, posX, posY) FROM '/graphNodesData.csv' DELIMITER ',' CSV HEADER;"      # load data in Facilities table
