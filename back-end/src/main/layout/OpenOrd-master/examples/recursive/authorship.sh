...
# adding command line parameter tuning
...
authorship.sh: a script to compute layout. Usage:
    ...
    -r | --refinecut REFINE_CUT: specify the REFINE_CUT variable for edge cutting. REFINE_CUT is the edge cutting parameter to use during refinement. Default .5.
    -f | --finalcut FINAL_CUT: specify the FINAL_CUT variable for edge cutting. FINAL_CUT is the edge cutting parameter to use during the last layout. Default .5."
...
if [ $LEVEL -gt 1 ]
then
  echo "cp" $ROOTNAME"_"$LEVEL".icoord" $ROOTNAME"_"$LEVEL".coarse_icoord"
